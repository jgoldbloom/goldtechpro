 ...
 */
function bowtie_form_install_configure_form_alter(&$form, $form_state) {
  $form['site_information']['site_name']['#default_value'] = 'Bowtie';
...
function _wr_profile_install_configure_form_submit($form, &$form_state) {
  ...
  drupal_set_message("Bowtie Easy Installation was a Success! Imported $count queries.");
  drupal_set_message("For security purposes, it's recommended that you delete $sql_file, or move the Bowtie profile folder out of your webroot.");
...
function _wr_profile_prefixTables($sql) {
  ...
  foreach ($prefixes as $key => $val) {
    $sql = strtr($sql, array('prefixbowtie_' . $key  => $val . $key));
  }
  // Then replace remaining tables with the default prefix.
  return strtr($sql, array('prefixbowtie_' => $defaultPrefix ));
}
